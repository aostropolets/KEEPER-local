...
Cohorts for kidney failure patients (CKD stage 1-4, ESRD, AKI, unknown) with subsequent sampling

variables: @dbo, @target for schemas; cohort ids are pre-specified but can be changed
*******/

create table @target.ckd_codes
...
insert into @target.ckd_codes (category,concept_id,concept_name,concept_code,vocabulary_id,domain_id )
select c.category,
      c.concept_id,
      c.concept_name,
      c.concept_code,
...
CREATE TABLE @target.creatinine (
	person_id INT,
...
SELECT cr.*,
    ...
    JOIN @target.CKD_codes on measurement_concept_id = concept_id and category = 'creatinine'
...
IF OBJECT_ID('height') IS NOT NULL
	DROP TABLE @target.height;
CREATE TABLE @target.height (
	person_id INT,
	measurement_date DATETIME2(6),
	measurement_concept_id INT,
	ht FLOAT,
...
INSERT INTO @target.height
    ...
    FROM @dbo.MEASUREMENT m
      JOIN @target.creatinine c on m.person_id=c.person_id
    WHERE m.measurement_concept_id IN (select concept_id
                                     from @target.ckd_codes
...
IF OBJECT_ID('EGFR') IS NOT NULL
	DROP TABLE @target.EGFR;
CREATE TABLE @target.EGFR (
	person_id INT,
	gender_concept_id INT,
	race_concept_id INT,
	measurement_date DATETIME2(6),
...
INSERT INTO @target.EGFR
    ...
    FROM (
           ...
           FROM @target.creatinine cr
	   JOIN @target.height h on cr.person_id = h.person_id
	    UNION
	    ...
	    join @target.CKD_codes on  measurement_concept_id = concept_id and category= 'egfr'
...
DELETE FROM @target.cohort where cohort_definition_id = 1000;
INSERT INTO @target.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
...
DELETE FROM @target.cohort where cohort_definition_id = 1001;
INSERT INTO @target.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
...
DELETE FROM @target.cohort where cohort_definition_id = 1002;
INSERT INTO @target.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
...
DELETE FROM @target.cohort where cohort_definition_id = 1003;
INSERT INTO @target.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
...
DELETE FROM @target.cohort where cohort_definition_id = 1004;
INSERT INTO @target.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
...
DELETE FROM @target.cohort where cohort_definition_id = 1005;
INSERT INTO @target.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
...
IF OBJECT_ID('GFRrecent') IS NOT NULL
	DROP TABLE @target.GFRrecent;

CREATE TABLE @target.GFRrecent (
	person_id INT NOT NULL
	,eGFRrecentVal FLOAT NOT NULL
	,eGFRrecentDt DATETIME2(6) NOT NULL
);

INSERT INTO @target.GFRrecent
...
CREATE TABLE @target.DialysisCooccurrentWithAKI (
	person_id INT,
...
INSERT INTO @target.DialysisCooccurrentWithAKI
...
FROM @target.cohort T
LEFT JOIN @target.cohort U
...
IF OBJECT_ID('GfrCooccurrentWithAcuteCondition') IS NOT NULL
	DROP TABLE  @target.GfrCooccurrentWithAcuteCondition;
CREATE TABLE  @target.GfrCooccurrentWithAcuteCondition(
	person_id INT,
	eGfrDt DATETIME2(6),
...
INSERT INTO @target.GfrCooccurrentWithAcuteCondition
...
CREATE TABLE @target.protein_stage (
	person_id INT,
	measurement_date DATETIME2(6),
	stage VARCHAR(100),
...
INSERT INTO  @target.protein_stage
    SELECT
      person_id,
      measurement_date,
    ...
    FROM
      (SELECT DISTINCT
         ...
         value_as_concept_id
       FROM @dbo.MEASUREMENT m
       JOIN @target.ckd_codes ON concept_id = measurement_concept_id AND category = 'albumin'
           ...
           FROM (
                  ...
                  FROM
                    (SELECT DISTINCT
                       ...
                       value_as_concept_id
                     FROM @dbo.MEASUREMENT m
                       join @target.ckd_codes on measurement_concept_id = concept_id
...
FROM ( select * FROM @dbo.MEASUREMENT join @target.ckd_codes on measurement_concept_id = concept_id
	and category ='protein') T1
JOIN
	(SELECT *
	FROM @dbo.MEASUREMENT
...
delete from @target.EGFR where Estage is null;
delete from @target.protein_stage where stage is null;
...
select count(*) from @target.EGFR;

CREATE INDEX idx_egfr_person_id ON @target.EGFR (person_id);
CREATE INDEX idx_protein_person_id ON @target.protein_stage (person_id);

/* Test stages */
SELECT * FROM (
SELECT *, ROW_NUMBER() OVER(PARTITION BY person_id, measurement_date ORDER by stage DESC) AS rn
 FROM (SELECT DISTINCT person_id, measurement_date, stage FROM @target.protein_stage) K1
 ) K1
 WHERE rn>1;
...
drop table @target.EGFR_b;



select top 50000 *
into @target.protein_stage_b
from @target.protein_stage;

drop table @target.protein_stage;

select  *
into @target.protein_stage
from @target.protein_stage_b;

drop table @target.protein_stage_b;
...
IF OBJECT_ID('UrineTestCloseToRntGfr') IS NOT NULL
	DROP TABLE @target.UrineTestCloseToRntGfr;
CREATE TABLE @target.UrineTestCloseToRntGfr (
	person_id INT
...
INSERT INTO @target.UrineTestCloseToRntGfr
    SELECT
      G.person_id,
    ...
    FROM @target.EGFR G
        JOIN @target.protein_stage U -- xxx initially left join
...
/*** Block 9B: A-staging: get most recent urine test and the prior urine test.
 ...
 IF OBJECT_ID('UrineTestCloseToRntGfr2') IS NOT NULL
	DROP TABLE @target.UrineTestCloseToRntGfr2;
CREATE TABLE @target.UrineTestCloseToRntGfr2 (
	 person_id INT
	,urineTestDt DATETIME2(6)
	,urineTestType VARCHAR(100)
	,urineTestNumVal FLOAT
...
INSERT INTO @target.UrineTestCloseToRntGfr2
...
, DATEDIFF(DAY, U.urineTestDt, J.urineTestDt)
 AS diffDay
FROM @target.UrineTestCloseToRntGfr U

LEFT JOIN (SELECT * FROM @target.UrineTestCloseToRntGfr WHERE rn = 1) J
...
INSERT INTO @target.Astaging
SELECT DISTINCT J1.person_id
,J1.Astage AS recentAstage
,J2.Astage AS priorAstage
FROM (SELECT * FROM @target.UrineTestCloseToRntGfr2 WHERE rn = 1) J1
LEFT JOIN (SELECT * FROM @target.UrineTestCloseToRntGfr2 WHERE rn =2) J2
ON J1.person_id = J2.person_id;


/* *** BLOCK 10: Get CKD case/control definition/algorithm related variables *** */
CREATE TABLE @target.AlgVar (
	person_id INT NOT NULL
...
/* recent eGFR NOT co-occurrent with Aki, Prerenal kideny injury, sepsis, volume depletion, shock */
	...
	FROM @target.GFRrecent G
	WHERE eGFRrecentDt NOT IN (SELECT DISTINCT eGfrDt FROM  @target.GfrCooccurrentWithAcuteCondition a
...
SELECT person_id
	...
	FROM @target.DialysisCooccurrentWithAKI
	GROUP BY person_id
	) J13
...
/* recent eGFR co-occurrent with Acute Condition (Aki, Prerenal kideny injury, sepsis, volume depletion, shock) */
	...
	FROM @target.UrineTestCloseToRntGfr2 G
	WHERE eGFRrecentDt IN (
		SELECT DISTINCT eGfrDt FROM  @target.GfrCooccurrentWithAcuteCondition A
	WHERE A.person_id=G.person_id)
	) J14
...
/* Has a urine test from the present to 24M before the most recent eGFR */
	...
	FROM @target.UrineTestCloseToRntGfr
	GROUP BY person_id
	) J19
ON J.person_id = J19.person_id

/* A staging */
LEFT JOIN  @target.Astaging J20
ON J.person_id = J20.person_id
;

/* *** BLOCK 11: Phenotyping *** */
DROP TABLE  @target.phenotypePre;
CREATE TABLE  @target.phenotypePre (
	person_id INT
	,transplantCnt INT
	,dialysisCnt INT
...
INSERT INTO  @target.phenotypePre
SELECT DISTINCT *
...
FROM @target.AlgVar A
) U
;


-- selected pts set
insert into @target.cohort
...
select distinct concept_id, concept_name, count(*) from @dbo.condition_occurrence co
join @dbo.concept on concept_id = condition_concept_id
 where person_id in
(1743047, 4009735, 3801986, 7581910, 5806023, 3489658, 2185909, 7608999, 5867417, 5556306, 6050622, 2062681, 4007527, 15885929, 8256784, 43742,
2367141, 35495, 35495, 5128674, 6050622, 2062681, 4007527, 15885929, 8256784, 43742, 2367141, 2484261, 4300298, 2119180, 4066434, 15959684,
9429417, 1814370, 32979, 18874, 21577, 22556, 22556, 320, 9207, 27953, 9907, 927, 29332, 22421, 24138, 2232, 1378, 3570, 1914, 25945, 13378, 7579, 15022)
and person_id not in (
select person_id from @dbo.condition_occurrence
where condition_concept_id in (46271022,193782,197320, 443597))
...
from @dbo.measurement m
join @dbo.measurement m2 on m.person_id = m2.person_id
join @dbo.condition_era c on c.person_id=m.person_id and datediff(year, condition_era_start_date, m.measurement_date)>1
join   @dbo.concept on condition_concept_id=concept_id and concept_name like '%lupus%'
and m.measurement_concept_id in (46236952,3049187,36306178,3053283)  and m2.measurement_concept_id in (46236952,3049187,36306178,3053283)
and datediff(year,m.measurement_date,m2.measurement_date)>1
and m.value_as_number<20 and m2.value_as_number<20;